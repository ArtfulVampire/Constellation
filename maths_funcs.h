...
#define ONE_RAD_IN_DEG 180. / M_PI //57.2957795
...
struct camera
{
    ...
    void rotateAboutTargetRL(const float& angle);
    void rotateAboutTargetUD(const float& angle);
    void rotateAboutDirection(const float& angle);
    void rotateAboutAxis(const vec3& axisIn, const float& angle);
    void zoom(const float& dist);

    void moveRL(const float& dist);
    void moveUD(const float& dist);
    void rotateFreeRL(const float& angle);
    void rotateFreeUD(const float& angle);
    void setTarget(const vec3& targ);
    vec3 getTarget();
