...
float* vec3::operator [](const int& num)
{
    return &v[num];
}

float* vec4::operator &(vec4& vec)
{
    return vec.v;
}

float vec4::operator [](const int& num)
{
    return v[num];
}
...
camera::camera() {}
camera::camera(vec3 position_, vec3 target_, vec3 up_)
{
    position = position_;
    target = target_;
    up = up_;
    ...
    camMatrix = look_at(position, target, up);

}

void camera::zoom(const float &dist)
{
...
void camera::setTarget(const vec3 &targ)
{
    target = targ;
    direction = target - position;
    distance = length(direction);
...
void camera::rotateAboutTargetRL(const float& angle)
{

    direction = right * (-sin(angle)) + direction * cos(angle);
    direction = normalise(direction); //opt

    position = target - direction * distance;

    right = cross(up, direction);
    right = normalise(right); //opt

    //optional. "up" is constant
    up = cross(direction, right);
    up = normalise(up);
...
void camera::rotateAboutTargetUD(const float& angle)
{

    direction = direction * cos(angle) - up * sin(angle);
    direction = normalise(direction); //opt
...
void camera::rotateAboutDirection(const float& angle)
{
...
void camera::rotateAboutAxis(const vec3& axisIn, const float& angle)
{
    vec3 axis = right * axisIn.v[0] + up * axisIn.v[1] - direction * axisIn.v[2];
    vec3 oldUp = up;
    vec3 newRight;
    newRight = axis;
    newRight -= up * dot(axis,up);
    newRight = normalise(newRight);
    float tempAng1 = acos(dot(newRight, right));
    tempAng1 *= ((dot(newRight, direction) > 0.)? 1.:-1.);
    rotateAboutTargetRL(tempAng1);
    float tempAng2 = acos(dot(axis, right));
    tempAng2 *= (dot(axis, up) > 0.)? -1.:1.;
    rotateAboutDirection(tempAng2);
    rotateAboutTargetUD(-angle);
    rotateAboutDirection(-tempAng2);
    rotateAboutTargetRL(-tempAng1);
}


void camera::moveRL(const float &dist)
{
    position += right*dist;
    setTarget(target + right*dist);
}

void camera::moveUD(const float &dist)
{
    position += up * dist;

    setTarget(target + up*dist);

}

void camera::rotateFreeRL(const float &angle)
{
    direction = direction * cos(angle) + right * sin(angle);
    right = cross(up, direction);
    right = normalise(right);
...
void camera::rotateFreeUD(const float &angle)
{
    direction = direction * cos(angle) + up * sin(angle);
    up = cross(direction, right);
    up = normalise(up); //opt
...
camera& camera::operator =(const camera& rcam)
{
    ...
    target = rcam.target;
    direction = rcam.direction;
    distance = rcam.distance;
    up = rcam.up;
    right = rcam.right;
    camMatrix = rcam.camMatrix;
}


void print(const camera& cam)
{
    printf("cam_pos = "); print(cam.position);
    printf("target = "); print(cam.target);
    printf("up = "); print(cam.up);
    printf("distance = %.1f\n\n", cam.distance);
}
